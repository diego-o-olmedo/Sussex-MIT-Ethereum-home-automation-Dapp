angular.module('app.directives', [])
            ...
            return {
                ...
                link: function ($scope, element, attrs, ngModel) {
                    ...
                    var isHex = function (hex) {
                        if (typeof hex == 'undefined' || typeof hex == '#null') {
                            return null;
                        }
    ...
    .directive('paletteItem', function () {
        return {
            restrict: 'E',
            template:
                '    <ion-modal-view style="" class=" " id="page7" title="Color Palette">\n' +
                '                                <ion-content overflow-scroll="true" padding="true" class="has-header">\n' +
                '                                    <div class="card">\n' +
                '                                        <div class="item item-divider">Palette2</div>\n' +
                '                                        <div class="item item-body">\n' +
                '                                                <material-picker ng-model="color" hover-model="hoverColor" ng-click="palette_color(color.hex)"\n' +
                '                                                                 size="15" ></material-picker>\n' +
                '                                                <div ng-bind="color.name"></div>\n' +
                '                                                <div class="colorpicker-preview" ng-style="{background: color.hex}"></div>\n' +
                '                                                <button class="btn btn-info" ng-click="color.hex=\'#FFFFFF\'">Set to White</button>\n' +
                ...
                '                                                <button class="btn btn-info" ng-click="color.hex=\'#null\'">init</button>\n' +
                '                                                <div style ng-show="hoverColor !== null">\n' +
                '                                                    Hex: {{hoverColor.hex}} Name: {{hoverColor.name}}\n' +
                '                                                    R: {{hoverColor.r}} G: {{hoverColor.g}} B: {{hoverColor.b}}\n' +
                '                                                </div>\n' +
                '                                        </div>\n' +
                '                                    </div>\n' +
                '                                </ion-content>\n' +
                '<ion-footer-bar ng-class = "::ui.headerFooterClass">\n ' +
                '<button class = "button button-stable" ng-click = "modal.hide()" > cancel </button> \n' +
                '                            </ion-modal-view>'
        }
    })


    .directive('sideMenuList', function () {
        return {
            restrict: 'E',
            template: '  <ion-list>\n' +

                // connect function -> move login page
                '                        <div ng-if="dataService.getConnectValue() == 0">\n' +
                '                        <ion-item href = "/#/page1/page10" ng-click="goLoginPage()"> Login</ion-item>\n' +
                '                        </div>\n' +

                // disconnect function -> logout and disconnect before move login page
                '                        <div ng-if="dataService.getConnectValue() == 1">\n' +
                '                        <ion-item ng-click="logoutEmail()"> LogOut</ion-item>\n' +
                '                        </div>\n' +

                // connection status
                '                        <p class="mid-title"> Current Status...</p>\n' +
                '                        <div ng-if="dataService.getConnectValue() == 1">\n' +
                '                            <p class = "Connect-font" > Connected </p>\n' +
                '                        </div>\n' +
                '                        <div ng-if="dataService.getConnectValue() == 0">\n' +
                '                            <p class = "disConnect-font" > DisConnected </p>\n' +
                '                        </div>\n' +
